...
export const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  const getTypeBadgeStyle = (type: string) => {
    switch (type) {
      ...
      case AccountType.ASSET: return 'bg-blue-950 text-blue-400 border-blue-800';
      case AccountType.LIABILITY: return 'bg-orange-950 text-orange-400 border-orange-800';
                      ...
                      {isEditing && !data.autoGenerated ? (
                        <div className="space-y-2">
                          <select value={data.type} onChange={(e) => handleChange('type', e.target.value)} className="w-full bg-slate-950 text-white border border-slate-700 rounded px-2 py-1 text-xs">
                            ...
                            <option value="ACTIF">Actif (Banque/Caisse)</option>
                            <option value="PASSIF">Passif (Dettes)</option>
