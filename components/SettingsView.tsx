...
import {
  ...
  ChevronDown, AlertTriangle, Briefcase, Users, ArrowRight, X, Terminal, CheckCircle
} from 'lucide-react';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ accounts, onUpdateAccounts }) => {
  ...
  const sortedAccounts = React.useMemo(() => {
    try {
      ...
      // Safe copy to avoid mutation issues
      const safeAccounts = [...accounts];

      // 1. Identify Parents and Children safely
      // Robust check for "empty" parentId (undefined, null, empty string, or explicit string "null")
      const isParent = (a: Account) => {
        if (!a.parentId) return true;
        const p = String(a.parentId).trim().toLowerCase();
        return p === "" || p === "null" || p === "undefined";
      };

      const parents = safeAccounts.filter(a => isParent(a));
      const children = safeAccounts.filter(a => !isParent(a));

      let result: Account[] = [];
      const processedIds = new Set<string>();

      // Sort parents by code (numerically if possible)
      parents.sort((a, b) => a.code.localeCompare(b.code, undefined, { numeric: true }));

      parents.forEach(parent => {
        result.push(parent);
        processedIds.add(parent.id);

        // Find children for this specific parent
        const myChildren = children.filter(c => c.parentId === parent.id);
        myChildren.sort((a, b) => a.code.localeCompare(b.code, undefined, { numeric: true }));
  ...
  const handleSave = () => {
    if (tempAccount) {
      ...
      const updatedAccounts = accounts.map(a => a.id === tempAccount.id ? tempAccount : a);
  ...
  const potentialParents = accounts.filter(a => a.id !== tempAccount?.id && !a.parentId);
          ...
          <tbody className="divide-y divide-slate-800">
            {accounts.length === 0 ? (
              ...
              sortedAccounts.map((account) => {
                ...
                // Strict check for parent status
                const isParent = !data.parentId || data.parentId === "null" || data.parentId === "undefined" || String(data.parentId).trim() === "";
                  ...
                  <tr key={account.id} className={`transition-colors ${rowClass}`}>
                    ...
                    <td className="px-6 py-4 align-top">
                      {isEditing ? (
                        ...
                        <div className={`w-10 h-10 rounded flex items-center justify-center border shadow-sm ${isParent ? 'bg-slate-700/50 border-slate-600 text-blue-400' : 'bg-slate-900 border-slate-800 text-slate-600'
                          }`}>
                          <IconComponent size={20} />
                        </div>
                      )}
                    ...
                    <td className="px-6 py-4 align-top">
                      {isEditing ? (
                        ...
                        <span className={`font-mono font-bold ${isParent ? 'text-white' : 'text-slate-500'}`}>{data.code}</span>
                      )}
                    </td>
